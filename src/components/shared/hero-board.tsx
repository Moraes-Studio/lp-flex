 ...
 * físico de uma academia de bairro, digitalizado. Depende do relógio do
 * visitante (dia da semana + aula rolando agora), então só computa depois
 * de montar no cliente pra não arriscar mismatch de hidratação.
 */
export function HeroBoard({ slots }: { slots: AulaSlot[] }) {
  ...
  React.useEffect(() => {
    const atualizar = () => {
      ...
      setEstado({ dia: DIA_JS_PARA_LABEL[agora.getDay()], minutos: agora.getHours() * 60 + agora.getMinutes() });
    };
    atualizar();
